...
def calcular():
    nome = request.form['nome'].strip()  # Remove espaços em branco no início e no fim

    # Verifica se o nome não está vazio
    if not nome:
        return render_template('resultado.html', error="Por favor, forneça um nome.")

    # Verifica se o nome contém números
    if any(char.isdigit() for char in nome):
        return render_template('resultado.html', error="O nome não deve conter números.")
    ...
    # Certifique-se de que seu template 'resultado.html' esteja preparado para exibir a mensagem de erro quando fornecida
    return render_template('resultado.html', resultados=resultados, soma_total=soma_total, vibracao_final=vibracao_final, error=None)
